...
<div class="row">
    <div class="col-md-12 card-box">
        <form class="parsley-examples" action="{{ route('visits.store') }}" method="POST">
            @csrf
            <h4 class="header-title">TAMBAH KUNJUNGAN</h4>
            <p class="sub-header"></p>

            <div class="form-group clearfix">
                <label for="patient" class="col-form-label control-label">Pasien<span class="text-danger">*</span></label>
                <div>
            ...
            <div class="form-group clearfix">
                ...
                <div>
                    <select class="form-control select2" id="penjamin-input" name="guarantator" required>
                        ...
                        <option value="{{ $guarantator }}" @if (old('guarantator')===$guarantator) selected @endif>
                            {{ $guarantator }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group clearfix">
                    ...
                    <div>
                        <input id="nomor-anggota" name="member_number" type="text" data-parsley-type="number"
                            class="form-control" value="{{ old('member_number') }}">
                    </div>
                    @error('member_number')
                    <ul class="parsley-errors-list filled">
                        <li>{{ $message }}</li>
                ...
                <div class="form-group clearfix">
                    ...
                    <div>
                        <input id="hak-kelas" name="access_rights" type="number" min="1" max="3"
                            data-parsley-type="number" class="form-control" value="{{ old('access_rights') }}">
                    </div>
                    @error('access_rights')
                    <ul class="parsley-errors-list filled">
                        <li>{{ $message }}</li>
            ...
            <div class="form-group clearfix">
                ...
                <div>
                    <select class="form-control select2" id="poli-tujuan" name="polyclinic" required>
                        ...
                        <option value="{{ $polyclinic->id }}" @if(old('polyclinic')===$polyclinic->id) selected
